...
pub(super) struct Channel<Signer: ChannelSigner> {
	...
	pub(super) consignment_endpoint: Option<ConsignmentEndpoint>,
...
impl<Signer: WriteableEcdsaChannelSigner> Channel<Signer> {
	...
	pub fn new_outbound<ES: Deref, SP: Deref, F: Deref>(
		...
		outbound_scid_alias: u64, consignment_endpoint: Option<ConsignmentEndpoint>, ldk_data_dir: PathBuf
	...
	pub fn new_from_req<ES: Deref, SP: Deref, F: Deref, L: Deref>(
		...
		current_chain_height: u32, logger: &L, outbound_scid_alias: u64, ldk_data_dir: PathBuf
		...
		L::Target: Logger
	{
		...
		let channel_monitor = ChannelMonitor::new(self.secp_ctx.clone(), monitor_signer,
		                                          ...
		                                          obscure_factor,
		                                          holder_commitment_tx, best_block, self.counterparty_node_id, self.ldk_data_dir.clone());
			...
			for htlc_update in htlc_updates.drain(..) {
				...
				match &htlc_update {
					&HTLCUpdateAwaitingACK::AddHTLC {amount_msat, cltv_expiry, ref payment_hash, ref source, ref onion_routing_packet, amount_rgb, ..} => {
						match self.send_htlc(amount_msat, *payment_hash, cltv_expiry, source.clone(), onion_routing_packet.clone(), false, logger, amount_rgb) {
...
impl<Signer: WriteableEcdsaChannelSigner> Writeable for Channel<Signer> {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		write_tlv_fields!(writer, {
			...
			(29, self.consignment_endpoint, option),
